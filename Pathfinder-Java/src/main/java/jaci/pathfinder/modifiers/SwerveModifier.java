...
/**
 * The Swerve Modifier will take in a Source Trajectory and spit out 4 trajectories, 1 for each
 * wheel on the drive. This is commonly used in robotics for robots with 4 individual wheels in a
 * 'swerve' configuration, where each wheel can rotate to a specified heading while still being
 * powered.
 * <p>
...
public class SwerveModifier {
    ...
    /**
     * The Swerve Mode to generate the trajectory with. This declares how the heading of the robot
     * is calculated based on the central trajectory.
     */
    public enum Mode {
        /**
         * SWERVE_DEFAULT will generate a trajectory with the robot constantly facing forward,
         * translating from side to side in order to follow a curved path
         */
        SWERVE_DEFAULT
    }
     ...
     *
     * @param wheelbase_width The width (in meters) between the individual left-right sides of the
     *                        drivebase
     * @param wheelbase_depth The width (in meters) between the individual front-back sides of the
     *                        drivebase
